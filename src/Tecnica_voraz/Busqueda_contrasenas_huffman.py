...
# Configuración
# -------------------------
# Carpeta por defecto que el programa intentará cargar al arrancar.
DEFAULT_DICT_FOLDER = os.path.join(os.path.dirname(__file__), "diccionarios")

# -------------------------
# Diccionario pequeño incluido de ejemplo
# -------------------------
SMALL_DICTIONARY = [
...
# -------------------------
# Huffman compression (texto completo)
# -------------------------
# Nota: en este caso codificamos el texto completo del fichero (palabras separadas por '\n'),
# lo comprimimos en memoria y guardamos el codebook. Para usar el diccionario lo decodificamos
# y hacemos splitlines() -> lista de palabras.
#
# Implementación clásica de Huffman para caracteres (suficiente para diccionarios de texto).
...
def text_to_bitstring(text, codes):
    # convierte texto a cadena de bits usando codes (map char->'01...')
...
def huffman_compress_text(text):
    # calcula frecuencias de caracteres sobre el texto (incluye '\n' separador)
    freqs = Counter(text)
    tree = build_huffman_tree(freqs)
    if tree is None:
        return b'', {}, 0
    codes = build_codes_from_tree(tree)  # char -> bitstring
...
def huffman_decompress_bytes(b, codes, pad_len):
    ...
    # construir mapa inverso: code -> char
    inv = {v: k for k, v in codes.items()}
    # decodificar por prefijo
    res_chars = []
    cur = ""
    for bit in bitstr:
        cur += bit
...
# -------------------------
# Utilidades de costes Huffman (para búsqueda voraz)
# -------------------------
...
class App(tk.Tk):
    def __init__(self):
        ...
        # botones y combobox para manejar carpeta de diccionarios
        ...
        # Divide tab (sin cambios funcionales)
        ...
        # lista interna de archivos detectados (cada entrada contiene compresión y metadata)
        # formato: {'path':..., 'name':..., 'compressed':bytes, 'codes':{char:code}, 'pad':int, 'size':int}
        self._dict_files = []

        # Al iniciar, cargar carpeta por defecto (si existe)
    ...
    # -------------------------
    # Carga / compresión / descompresión
    # -------------------------
    def load_default_dictionary_folder(self):
        """
        Se ejecuta al arrancar: intenta cargar DEFAULT_DICT_FOLDER si existe.
        """
    ...
    def decompress_entry(self, entry):
        """
        Devuelve lista de palabras del entry, descomprimiéndolo.
        """
        if not entry:
            return []
        try:
